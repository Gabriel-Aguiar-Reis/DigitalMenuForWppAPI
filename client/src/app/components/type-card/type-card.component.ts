...
export class TypeCardComponent implements OnInit {
    ...
    constructor(private typeService: TypeService, private productService: ProductService) {}
    ...
    loadProducts() {
      this.productService.getProducts().subscribe(
        (response) => {
          this.products = response.filter((product: { type: string; }) => product.type === this.typeId);
        }
      );
    }
  }
